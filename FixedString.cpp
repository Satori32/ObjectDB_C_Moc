...
FixedString<N> StringFormater<N>(char* Format, ...) {
	...
	FixString<N> S = ConstructString(Buff, strlen(Buff));
...
FixString<A> MakeSingle(FixedUTF8<B>& In) {
	FixString<A> F;
...
template<size_t N>
FixedString<N> ConstructFixString<N>(char* S, size_t L) {
	FixString<N> F;
...
FixString<A + B + 2> Add(FixString<A>& A, FixString<B>& B) {
	FixString<A + B + 2> C = ConstructFixedString<A + B + 2>(A.S, strlen(A.S));
	strcat(GetPTR(C), C.L);
