...
template <size_t A, size_t B> FixString<A> MakeSingle(FixedUTF8<B>& In);
...
template<size_t N> FixedString<N> ConstructFixString<N>(char* S, size_t L);
template<size_t N> size_t Length(FixString<N>& In);
template<size_t N> char* GetPTR(FixString<N>& In);
template<size_t A, size_t B> FixString<A + B + 2> Add(FixString<A>& A, FixString<B>& B);
